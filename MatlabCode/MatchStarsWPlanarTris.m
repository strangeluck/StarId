...
function Results = MatchStarsWPlanarTris( StarsInFOV, climit, plimit, sigm, sigx );
...
if nStarsInFOV < 3
    ...
    if nCombs > 0         
        ...
        Prop2min = T(start).Prop2 - sigx * sqrt( var );
        Prop2max = T(start).Prop2 + sigx * sqrt( var );
...
%         WaitForKeyPress;  % Use to stop at every pivot
        ...
        T(k).Prop2 = PlanarTriPolarMoment( sv1, sv2, sv3 );
        var = StarPlanarMomentCov( sv1, sv2, sv3, sigm );
        Prop2min = T(k).Prop2 - sigx * sqrt(var);
        Prop2max = T(k).Prop2 + sigx * sqrt(var);
